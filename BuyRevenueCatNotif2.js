...
function getAppName(ua) {
  ...
  const matched = keys.find(k => ua.includes(k));
  if (matched) {
    return decodeURIComponent(matched).replace(/\/$/, '');
  }
  return 'Unknown App';
}

function notifySuccess(appName, entitlement) {
  $notification.post(
    '✅ RevenueCat Unlocked',
    `App: ${appName}`,
...
function notifyFailure(ua) {
  $notification.post(
    '❌ RevenueCat Not Matched',
    `No mapping found`,
...
$done({'body':JSON[_0x2534be(0xbb,'v%bD')](obj)});var version_ = 'jsjiami.com.v7';
