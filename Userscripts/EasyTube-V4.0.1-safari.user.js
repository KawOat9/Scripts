// ==UserScript==
// @name               EasyTube V4.0.1 — Ad Skip & SponsorBlock & HD Download⬇️🚀 (No Lag)
// @name:th            EasyTube V4.0.1 — ข้ามโฆษณา, SponsorBlock และดาวน์โหลด HD (ไม่กระตุก)

// @description        V4.0.1 — Rewritten for performance. 🚫 Multi-layer ad skip with minimal CPU usage. ⏭ SponsorBlock (9 cats). 🎥 Force 4K. ⬇ Download via evdfrance.fr. 💾 Remembers settings. Bypass adblock detection. No Cobalt. No lag.
// @description:th     V4.0.1 — เขียนใหม่เพื่อประสิทธิภาพ 🚫 ข้ามโฆษณาทันที ⏭ SponsorBlock 🎥 4K ⬇ ดาวน์โหลดผ่าน evdfrance.fr 💾 บันทึกการตั้งค่า ไม่กระตุก

// @namespace          https://greasyfork.org/users/1510019
// @version            4.0.1-safari
// @author             2pixel (rewrite: performance-optimized) | Safari/iOS patch
// @license            MIT
...
(function patchYT() {
    ...
    // Safari may reject defineProperty on Object.prototype in strict contexts — guard carefully
    try {
        const desc = Object.getOwnPropertyDescriptor(Object.prototype, 'adPlacements');
        if (!desc) {
...
// ═══════════════════════════════════════════════════════════════════════════
// GM API SHIM — Safari/iOS compatibility (GM_getValue/GM_setValue may be absent)
// ═══════════════════════════════════════════════════════════════════════════
(function shimGM() {
    if (typeof GM_getValue === 'undefined') {
        window.GM_getValue = function(key, def) {
            try { const v = localStorage.getItem('__et4_' + key); return v === null ? def : JSON.parse(v); } catch { return def; }
        };
    }
    if (typeof GM_setValue === 'undefined') {
        window.GM_setValue = function(key, val) {
            try { localStorage.setItem('__et4_' + key, JSON.stringify(val)); } catch {}
        };
    }
    ...
    if (typeof GM_xmlhttpRequest === 'undefined') {
        window.GM_xmlhttpRequest = function(opts) {
            ...
            xhr.onload  = () => opts.onload  && opts.onload({ status: xhr.status, responseText: xhr.responseText });
            xhr.onerror = () => opts.onerror && opts.onerror();
            try { xhr.send(); } catch(e) { opts.onerror && opts.onerror(e); }
        };
    }
})();
...
// SponsorBlock check — use setInterval instead of rAF loop for iOS compatibility
// (rAF freezes in background tabs on iOS Safari, causing SB to stop working)
let _sbInterval = null;
function startSBLoop() {
    if (_sbInterval) return;
    _sbInterval = setInterval(_doCheckSB, 400);
}
// Keep rAF entry point for boot() compatibility but delegate to interval
function checkSB() {
    // no-op: loop is handled by setInterval above
}
...
function downloadVideo(videoId) {
    ...
    // Safari/iOS blocks window.open() — use <a> click instead
    try {
        const a = document.createElement('a');
        a.href = `https://evdfrance.fr/convert/?id=${videoId}`;
        a.target = '_blank';
...
function buildPanel() {
    ...
    info.appendChild(mk('span', null, 'V4.0.1: Safari/iOS patch — touch drag, GM shim, setInterval SB, popup-safe download.'));
    body.appendChild(info);

    panel.appendChild(body);
    panel.appendChild(mk('div', 'e4-foot', `EasyTube V4.0.1-safari · 2pixel · 2025`));
...
function initDrag(panel) {
    ...
    function moveDrag(clientX, clientY) {
        ...
        raf = true;
        requestAnimationFrame(() => {
            ox = Math.max(8, Math.min(window.innerWidth  - pw - 8, clientX - ix));
            oy = Math.max(8, Math.min(window.innerHeight - ph - 8, clientY - iy));
            panel.style.transform = `translate3d(${ox}px,${oy}px,0)`;
            raf = false;
        });
    }
    ...
    hdr.addEventListener('pointerup',     e => { if (e.pointerId !== pid) return; endDrag(); }, { passive: true });
    hdr.addEventListener('pointercancel', e => { if (e.pointerId !== pid) return; endDrag(); }, { passive: true });
...
.e4-hdr { -webkit-user-select:none; user-select:none; cursor:move; touch-action:none; }
...
function boot() {
    ...
    // Start SponsorBlock loop (setInterval-based for iOS Safari)
    startSBLoop();
