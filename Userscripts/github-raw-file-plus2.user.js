...
function appendButtons(fileList, isTable = false) {
    ...
    for (let i = 0; i < fileList.length; i++) {
        ...
        // แปลง GitHub URL หลายรูปแบบให้เป็น raw.githubusercontent.com
        if (fileUrl.includes('/blob/') || fileUrl.includes('/edit/')) {
            // /blob/ และ /edit/ แปลงเป็น raw URL
            rawFileUrl = fileUrl
                .replace('https://github.com/', 'https://raw.githubusercontent.com/')
                .replace(/\/(blob|edit)\//, '/');
        } else if (fileUrl.includes('/releases/download/')) {
            // release asset ใช้ URL ตรง ๆ
            rawFileUrl = fileUrl;
        } else {
            // อื่น ๆ ใช้ URL ตรง ๆ
            rawFileUrl = fileUrl;
        }
