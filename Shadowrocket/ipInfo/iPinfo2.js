...
$httpClient.get({ url, headers }, (error, response, data) => {
  if (error) {
    const htmlError = `<h2>🔎 IPPure - Connect Error</h2><p>โปรดตรวจสอบเครือข่าย</p>`;
  ...
  try {
    ...
    // ใช้ค่าละติจูดและลองจิจูด ถ้า API ไม่มีให้ default ไปศูนย์โลก
    ...
    const html = `
      ...
      <head>
        ...
        <style>
          ...
          .card { background: #fff; padding: 20px; border-radius: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); max-width: 600px; margin: auto; }
    ...
    const htmlError = `<h2>🔎 IPPure - Parse Error</h2><p>${e.message}</p>`;
    $done({ response: { status: 200, headers: { "Content-Type": "text/html;charset=utf-8" }, body: htmlError } });
  }
});
