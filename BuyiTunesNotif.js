...
^https?:\/\/buy\.itunes\.apple\.com\/verifyReceipt$ url script-response-body https://raw.githubusercontent.com/KawOat9/Scripts/main/BuyiTunes.js
...
function getAppInfo(bundleId) {
  const map = productidmap[bundleId] || productidmap['default'];
  return {
    name: map[0],
    product: map[1]
  };
...
function notifySuccess(appName, productId) {
  $notification.post(
    '✅ BuyiTunes Unlocked',
    `App: ${appName}`,
    `Product: ${productId}`
...
function notifyFailure(bundleId) {
  $notification.post(
    '❌ BuyiTunes Not Matched',
    'No mapping found',
...
try {
  const bundleId =
    (ddgksf2013 &&
     ddgksf2013.receipt &&
     ddgksf2013.receipt.bundle_id) || '';
  
  if (productidmap[bundleId]) {
    const info = getAppInfo(bundleId);
    ...
    try {
      const inapps = (
       ...
       ).map(i => i.product_id).slice(0,3);

      $notification.post(
        '🔍 Real Product IDs',
        inapps.length ? inapps.join(', ') : '(none)',
        `BundleID: ${bundleId || 'Unknown'}`
      );

    } catch(e2) {}

    notifyFailure(bundleId);
  }

} catch(e) {}

$done({'body':JSON[_0x3dd319(0x14b,'5Cte')](ddgksf2013)});var version_ = 'jsjiami.com.v7';